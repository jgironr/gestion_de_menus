...
export default function Menus() {
  ...
  const [selectedMenuDetails, setSelectedMenuDetails] = useState<Menu | null>(
    null
  );
  ...
  const agregarProductoAComplemento = (
    ...
    const complementosActualizados = selectedMenu.complementos.map(
      (complemento) => {
        if (complemento.id === complementoId) {
          const productoExistente = complemento.productos.find(
            (p) => p.id === productoId
          );
          if (productoExistente) {
            showNotificationMessage(
              "Este producto ya ha sido añadido al complemento."
            );
            return complemento;
          }
          return {
  ...
  const actualizarCantidadProductoDeComplemento = (
    complementoId: number,
    productoId: number,
  ...
  const eliminarProductoDeComplemento = (
    complementoId: number,
    productoId: number
  ) => {
    if (!selectedMenu) return;
    ...
    const complementosActualizados = selectedMenu.complementos.map(
      (complemento) => {
        if (complemento.id === complementoId) {
          const productosActualizados = complemento.productos.filter(
            (p) => p.id !== productoId
          );
          return { ...complemento, productos: productosActualizados };
        }
  ...
  const agregarProductoAlMenu = (productoId: number) => {
    ...
    const productoExistente = selectedMenu.productos.find(
      (p) => p.id === productoId
    );
    if (productoExistente) {
  ...
  const calcularCostoTotal = (): number => {
    const costoProductos =
      selectedMenu?.productos?.reduce((total, p) => {
        const costo = obtenerCosto(p);
        return total + costo * (p.cantidad ?? 1);
      }, 0) || 0;

    const costoComplementos =
      selectedMenu?.complementos?.reduce(
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(option) =>
                      option && agregarProductoAlMenu(option.value)
                    }
                      ...
                      {complemento.productos.map((producto) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            min="1"
                            onChange={(e) =>
                              actualizarCantidadProductoDeComplemento(
                                complemento.id,
                                producto.id,
                                parseInt(e.target.value)
                              )
                ...
                {selectedMenuDetails.productos.map((prod) => (
                  <li key={prod.id} className="flex justify-between">
                    <span>
                      ...
                      (Compra: Q{prod.producto?.costo || 0} - Reventa: Q
                      {prod.producto?.unidad || 0}) (x{prod.cantidad})
                    </span>
                    <span>
                      Total: Q
                      {((prod.producto?.unidad || 0) * prod.cantidad).toFixed(
                        2
                      )}
                    </span>
                  </li>
            ...
            <div className="mt-6">
              ...
              {selectedMenuDetails.complementos.length > 0 ? (
                      ...
                      {complemento.productos.map((p) => (
                        <li key={p.id} className="flex justify-between">
                          <span>
                            ...
                            (Compra: Q{p.producto?.costo || 0} - Reventa: Q
                            {p.producto?.unidad || 0}) (x{p.cantidad})
                          </span>
                          <span>
                            Total: Q
                            {((p.producto?.unidad || 0) * p.cantidad).toFixed(
                              2
                            )}
                          </span>
                        </li>
                ...
                <p className="text-gray-500 italic">
                  No se han agregado complementos.
                </p>
              )}
            </div>
            ...
            <div className="mt-6">
                  ...
                  selectedMenuDetails.complementos.reduce(
                    (totalComplemento, complemento) =>
                      ...
                      complemento.productos.reduce(
                        (total, p) =>
                          total + (p.producto?.costo ?? 0) * p.cantidad,
                        0
                      ),
                    0
                  )
              ...
              <p>
                ...
                {(
                  selectedMenuDetails.productos.reduce(
                    (total, p) =>
                      total + (p.producto?.unidad ?? 0) * p.cantidad,
                    0
                  ) +
                  selectedMenuDetails.complementos.reduce(
                    (totalComplemento, complemento) =>
              ...
              <p>
                ...
                {(
                  selectedMenuDetails.productos.reduce(
                    (total, p) =>
                      total +
                      ((p.producto?.unidad ?? 0) - (p.producto?.costo ?? 0)) *
                        p.cantidad,
                    0
                  ) +
                  selectedMenuDetails.complementos.reduce(
              ...
              >
                        ...
                        {menu.productos.map((p) => {
                          ...
                          return (
                            <li key={producto.id} className="ml-4">
                              {producto.descripcion} (x{p.cantidad}) - Compra: Q
                              {producto.costo} / Reventa: Q{producto.unidad}
                            </li>
                          );
                        })}
                      </>
                        ...
                        {menu.complementos.map((complemento) => (
                          ...
                          >
                            {complemento.productos.map((p) => {
                              ...
                              return (
                                <li key={producto.id}>
                                  {producto.descripcion} (x{p.cantidad || 1}) -
                                  Compra: Q{producto.costo} / Reventa: Q
                                  {producto.unidad}
                                </li>
                              );
                            })}
                ...
                <td className="p-4 text-sm">
                  ...
                  {(
                    ...
                    (menu.complementos || []).reduce(
                      (totalComp, complemento) => {
                        return (
                          totalComp +
                          (complemento.productos || []).reduce((total, p) => {
                            const producto = p.producto || p;
                  ...
                  selectedMenu.productos.map((p) => (
                    ...
                    >
                      <span>
                        ...
                        (Compra: Q{p.producto?.costo || p.costo} - Reventa: Q
                        {p.producto?.unidad || p.unidad}) (x{p.cantidad})
                      </span>
                      <input
                        type="number"
                        value={p.cantidad}
                    ...
                    <ul>
                      {complemento.productos.map((p) => {
                        ...
                        return (
                          ...
                          >
                            <span>
                              {descripcion} (Compra: Q{costo} - Reventa: Q
                              {unidad}) (x{p.cantidad})
                            </span>
                            <input
                              type="number"
