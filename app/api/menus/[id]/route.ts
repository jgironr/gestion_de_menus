...
export async function PUT(
  ...
  try {
    ...
    const productos: Array<{ id: number; cantidad: number }> = Array.isArray(
      data.productos
    )
      ? data.productos
      : [];
    ...
    }> = Array.isArray(data.complementos) ? data.complementos : [];

    const productoIds = productos.map((p) => p.id);
    const complementoProductoIds = complementos.flatMap(
      (c) => c.productos?.map((p) => p.id) || []
    );
    ...
    const idsNoEncontrados = todosProductosIds.filter(
    ...
    return NextResponse.json(
      { error: "Hubo un problema al actualizar el menú" },
      { status: 500 }
    );
  }
}
